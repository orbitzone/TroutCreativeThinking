var isMobile = {
    ...
    play: function(container, mute) {
        if (this.exists(container)) {
            ...
            playerManager.quality(container, "hd1080"), "undefined" != typeof mute && (this.players[container].settings.mute = mute);
    ...
    quality: function(container, quality) {
        "undefined" == typeof quality && (quality = "hd1080"), this.players[container].player.setPlaybackQuality(quality);
    },
    ...
    onChangeState: function(event) {
        for (var id, alphabet = [ "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z" ], i = 0; i < alphabet.length; i++) if ("undefined" != typeof event.target[alphabet[i]].id) {
            id = event.target[alphabet[i]].id;
            break;
        }
        var current = this.players[id];
        "undefined" != typeof current.settings && (event.data === YT.PlayerState.ENDED && 1 == current.settings.loop && this.players[current.settings.container].player.playVideo(), 
    ...
    inspiration: function() {
        ...
        if ($(window).on("resize", function() {
            var height = $(".blocks .water-therapy .content").height() + 10, width = 16 * height / 9;
    ...
    waterTherapy: function() {
        ...
        $("#water-therapy-rejuvenation-video").addClass("ready"), isMobile.any() && $("#water-therapy-rejuvenation-video").remove(); else for (var initialVideoLoad = [ "water-therapy", "water-therapy-rejuvenation", "water-therapy-relaxation", "water-therapy-therapeutic" ], k = 0; k < initialVideoLoad.length; k++) {
        ...
        if (video) {
            var player_container = "water-therapy-full-video", vars = {
                showinfo: 0,
                modestbranding: 0,
                rel: 0,
                controls: 0
            ...
            bannerVideo.init({
                ...
                loop: !1,
                onReady: function() {
                    $("#banner .banner").hasClass("playing") && !deviceMobile && playerManager.play(player_container), 
                ...
                onEnded: function() {
                    ...
                    $("#banner .banner").removeClass("playing"), $("#banner .banner").height("514px"), 
                    $("#banner .stop-full-video").removeClass("show"), $("#banner .play-full-video").removeClass("paused playing"), 
        ...
        $("#banner .play-full-video").on("click", function() {
            var player_container = "water-therapy-full-video";
            $("#banner .banner").addClass("playing"), deviceMobile || $("#banner .banner").height($("#water-therapy-full-video").height()), 
            $(this).hasClass("playing") ? ($(this).removeClass("playing"), $(this).addClass("paused"), 
            playerManager.pause(player_container)) : ($("#full-video-popup").removeClass("hide mfp-hide"), 
            $("#banner .stop-full-video").addClass("show"), $(this).addClass("playing"), $("#water-therapy-full-video").show(), 
            playerManager.play(player_container), ($(window).width() < 768 || deviceMobile) && $.magnificPopup.open({
                ...
                callbacks: {
                    ...
                    afterClose: function() {
                        $("#banner .banner").height("514px"), $("#banner .banner").removeClass("playing"), 
                        $("#banner .stop-full-video").removeClass("show"), $("#banner .play-full-video").removeClass("paused playing"), 
                        $("#full-video-popup").addClass("hide"), playerManager.stop(player_container);
                    }
                }
            }));
            ...
            var section = $(this).text().toLowerCase(), topPosition = parseInt($("#intro .video-wrap").offset().top - 50), animateToPlay = function() {
                if (topPosition = parseInt($("#intro .video-wrap").offset().top - 50), deviceMobile) $("html, body").animate({
                    ...
                    var scrollMenuAnimation = function() {
                        ...
                        setTimeout(function() {
                            ...
                            $.each(sections, function(key, val) {
                                val !== section && (console.log(val), playerManager.pause("water-therapy-" + val + "-video"));
                            });
                        }, 10);
                    };
            ...
            $("#banner .banner").hasClass("playing") ? ($("#banner .stop-full-video").trigger("click"), 
            setTimeout(function() {
                animateToPlay();
            }, 1e3)) : animateToPlay();
        ...
        $("#shower-technology .shower-technology-menu a").on("click", function() {
            ...
            $("#shower-technology .st-" + section).addClass("active")) : initialLoad !== !0 ? $("html, body").animate({
                scrollTop: posTop
            }, 500, function() {
                $("#shower-technology .shower-technology-card").removeClass("active"), $("#shower-technology .st-" + section).addClass("active");
            }) : initialLoad = !1 : ($("#shower-technology .shower-technology-card").removeClass("active"), 
            $("#shower-technology .st-" + section).addClass("active")), "st" == playerManager.active.substring(0, 2) && playerManager.stop(playerManager.active), 
        ...
        if ($(window).on("resize", function() {
            ...
            514 > height && (height = 514, width = 16 * height / 9), $("#water-therapy-video").width(width), 
            $("#water-therapy-video").height(height), //height = $('#banner .banner').height();
            //var width = 16 * height/9;
            deviceMobile ? (width = $(window).width(), height = $(window).height()) : $("#banner .banner").hasClass("playing") && $("#banner .banner").height(height), 
            $("#water-therapy-full-video").height(height), $("#water-therapy-full-video").width(width);
